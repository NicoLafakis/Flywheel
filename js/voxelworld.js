...
export class VoxelWorld3D {
  ...
  update(dt, events) {
    ...
    const standingCount = this.sim.remainingBlocksCount ?? 0;
    ...
    for (const ev of events || []) {
      if (ev.type === 'coin') {
        ...
        const puColor = ev.powerup && ev.powerup.spec ? ev.powerup.spec.color : 0x00d2ff;
        this.spawnPowerUpCollectBurst(ev.powerup ? ev.powerup.x : h.x, ev.powerup ? ev.powerup.z : h.z, puColor);
        ...
        this.spawnDustPuff(ev.powerup ? ev.powerup.x : 0, ev.powerup ? ev.powerup.z : 0, 0.9, 0, 0);
      } else if (ev.type === 'powerup_spawn') {
        this._addPowerUpMesh(ev.powerup);
        const puColor = ev.powerup && ev.powerup.spec ? ev.powerup.spec.color : 0x00d2ff;
        this.spawnPowerUpSpawnBeams(ev.powerup ? ev.powerup.x : 0, ev.powerup ? ev.powerup.z : 0, puColor);
  ...
  triggerTransformationLighting({ color = 0x00f0ff, duration = 2.0 } = {}) {
    if (!this._transLight && this.scene) {
      this._transLight = new THREE.PointLight(color, 0, 50, 1.2);
      this.scene.add(this._transLight);
    }
    if (this._transLight) {
      this._transLight.color.set(color);
      this._transLight.intensity = 8.5;
  ...
  spawnShockwaveRing(x, z, { color = 0x00f0ff, maxRadius = 35.0, duration = 1.2 } = {}) {
  ...
  spawnOrbitalBeacon(x, z, { color = 0x00f0ff, duration = 8.0 } = {}) {
    const beaconGeo = new THREE.CylinderGeometry(0.5, 2.5, 100, 16, 1, true);
    const beaconMat = new THREE.MeshBasicMaterial({
      color,
      transparent: true,
      opacity: 0.75,
    ...
    beaconMesh.position.set(x, 50, z);
