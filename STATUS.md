...
- 2026-08-16 — Level 1 (The Lab) 6-Player Invite Lobby Multiplayer Shipped:
  ...
  - **Multi-Client Wire Protocol & Supabase Realtime Transport**: Authoritative host (`MultiplayerHost`, `js/multiplayer/host.js`) and client peer (`MultiplayerPeer`, `js/multiplayer/peer.js`) over **Supabase Realtime Broadcast** (`LiveBroadcastChannel`, `js/multiplayer/channel.js`), providing robust cross-device internet multiplayer (with a `typeof window` Node test-suite exclusion).
  - **Multiplayer UI, 3D Rival Hole Rendering, and Podium**: Added `MultiplayerUI` (`js/multiplayer/ui.js`), `css/multiplayer.css` (with mobile scroll fixes), in-world rival colored hole meshes with neon rims in `VoxelWorld3D` (`js/voxelworld.js`), title screen `👥 MULTIPLAYER (2-6P)` button, and post-match victory podium with rankings.
