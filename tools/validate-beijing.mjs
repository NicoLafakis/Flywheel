// Beijing Forbidden City & Bird's Nest — standalone scene validator.
//
// The fast iteration loop: builds Beijing in ~1-2 s with full invariant assertions.
import { VoxelSandboxSim, loadScene, SCENE_GOALS } from '../js/voxelsim.js';
import { CITY_CATALOG } from '../js/citycatalog.js';
import {
  BEIJING_CROSSINGS, BEIJING_OPEN_GROUND, BEIJING_ROAD_SPANS,
  BEIJING_STREETS, BEIJING_VEHICLES,
} from '../js/voxelscene-beijing.js';
import { readFileSync } from 'node:fs';

await loadScene('beijing');
const newSim = () => new VoxelSandboxSim({ seed: 'validator', scene: 'beijing' });

const DECLARED = CITY_CATALOG.find((c) => c.scene === 'beijing')?.blocks || 38000;
...
function probeDeclaredCount(sim) {
  ...
  if (built !== DECLARED) {
    fail(`beijing: built ${built} blocks, catalog declares ${DECLARED} (delta ${built - DECLARED > 0 ? '+' : ''}${built - DECLARED})`);
  } else {
    pass(`block count: built=${built} catalog=${DECLARED} (exact 38,000)`);
  }
}

function probeCameraBlockers(sim, tops) {
  if (!sim.cameraBlockers || sim.cameraBlockers.length === 0) {
    fail('beijing: cameraBlockers is empty — did the builder discard generateBlockers\' return value?');
    return;
  }
  ...
  if (uncovered > 0) {
    fail(`beijing: ${uncovered} footprint cell(s) >=6 m tall have no cameraBlocker covering their height (tallest ${worstH} m at cell ${worstCell})`);
  } else {
...
function probeCellOwnership(sim) {
  ...
  if (ghosts > 0) {
    fail(`beijing: ${ghosts} fine cells owned by the wrong block (overlapping placement)`);
    let shown = 0;
...
function probePureSimBoundary() {
  const src = codeOnly(readFileSync(new URL('../js/voxelscene-beijing.js', import.meta.url), 'utf8'));
  const bad = [];
  ...
  if (bad.length) fail(`beijing: pure-sim boundary violated — ${bad.join(', ')}`);
...
function probeDeadImports() {
  ...
  if (dead.length) fail(`beijing: ${dead.length} dead voxelkit import(s) — ${dead.join(', ')}`);
...
function probeIdleStability(sim) {
  ...
  if (fallen.length > 0) {
    fail(`beijing: ${fallen.length} block(s) fell under gravity during idle (floating / unsupported geometry)`);
...
function probeDeterminism() {
  ...
  if (a.blocks.length !== b.blocks.length) {
    fail(`beijing: two builds differ in block count (${a.blocks.length} vs ${b.blocks.length})`);
    return;
  }
  const fa = fingerprint(a), fb = fingerprint(b);
  if (fa !== fb) fail(`beijing: two builds differ structurally (fingerprint ${fa} vs ${fb})`);
...
function probeWinnable(sim) {
  ...
  if (left > 0) fail(`beijing: ${left} block(s) survived a full clear — the harness is not clearing the city`);
  else if (!sim.won) {
    fail(`beijing: consumed all ${sim.blocks.length} blocks and won is still false (shortfall ${shortfall.toExponential(3)} vs epsilon ${epsilon.toExponential(3)})`);
  } else {
...
console.log('--- Validating Beijing Forbidden City & Bird\'s Nest ---');
const sim = newSim();
...
console.log(`\n  beijing sandbox: blocks=${sim.blocks.length} mass=${Math.round(sim.blocks.reduce((acc, b) => acc + (b.mass || 1), 0))} blockers=${sim.cameraBlockers.length}`);
