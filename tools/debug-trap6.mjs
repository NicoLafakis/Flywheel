import { VoxelSandboxSim, loadScene } from '../js/voxelsim.js';

await loadScene('boston');

const DT = 1 / 60;
const sim = new VoxelSandboxSim({ seed: 'probe', scene: 'boston' });
sim.tune.contactBudget = 12;
const h = sim.hole;
h.x = -96; h.z = -56; h.radius = 3.2;
h.rawMass = 800;

for (const b of sim.blocks) {
  let internalJam = b._jamReady;
  Object.defineProperty(b, '_jamReady', {
    get() { return internalJam; },
    set(val) {
      if (val === true && !internalJam) {
        if (b.id === 3858) {
          console.log(`Block 3858 _jamReady became true! Base: ${b.y - b.sy / 2}`);
        }
      }
      internalJam = val;
    }
  });
}

for (let i = 0; i < 20 * 60; i++) {
  sim.step(DT, { x: 0, z: 0 });
}
console.log('Done.');
