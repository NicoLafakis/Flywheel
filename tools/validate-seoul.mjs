// Seoul Han River & N Seoul Tower — standalone scene validator.
//
// The fast iteration loop: builds Seoul in ~1-2 s with full invariant assertions.
import { VoxelSandboxSim, loadScene, SCENE_GOALS } from '../js/voxelsim.js';
import { CITY_CATALOG } from '../js/citycatalog.js';
import {
  SEOUL_CROSSINGS, SEOUL_OPEN_GROUND, SEOUL_ROAD_SPANS,
  SEOUL_STREETS, SEOUL_VEHICLES,
} from '../js/voxelscene-seoul.js';
import { readFileSync } from 'node:fs';

await loadScene('seoul');
const newSim = () => new VoxelSandboxSim({ seed: 'validator', scene: 'seoul' });

const DECLARED = CITY_CATALOG.find((c) => c.scene === 'seoul')?.blocks || 32000;
...
function probeDeclaredCount(sim) {
  ...
  if (built !== DECLARED) {
    fail(`seoul: built ${built} blocks, catalog declares ${DECLARED} (delta ${built - DECLARED > 0 ? '+' : ''}${built - DECLARED})`);
  } else {
    pass(`block count: built=${built} catalog=${DECLARED} (exact 32,000)`);
  }
}

function probeCameraBlockers(sim, tops) {
  if (!sim.cameraBlockers || sim.cameraBlockers.length === 0) {
    fail('seoul: cameraBlockers is empty — did the builder discard generateBlockers\' return value?');
    return;
  }
  ...
  if (uncovered > 0) {
    fail(`seoul: ${uncovered} footprint cell(s) >=6 m tall have no cameraBlocker covering their height (tallest ${worstH} m at cell ${worstCell})`);
  } else {
...
function probeCellOwnership(sim) {
  ...
  if (ghosts > 0) {
    fail(`seoul: ${ghosts} fine cells owned by the wrong block (overlapping placement)`);
    let shown = 0;
...
function probePureSimBoundary() {
  const src = codeOnly(readFileSync(new URL('../js/voxelscene-seoul.js', import.meta.url), 'utf8'));
  const bad = [];
  ...
  if (bad.length) fail(`seoul: pure-sim boundary violated — ${bad.join(', ')}`);
...
function probeDeadImports() {
  ...
  if (dead.length) fail(`seoul: ${dead.length} dead voxelkit import(s) — ${dead.join(', ')}`);
...
function probeIdleStability(sim) {
  ...
  if (fallen.length > 0) {
    fail(`seoul: ${fallen.length} block(s) fell under gravity during idle (floating / unsupported geometry)`);
...
function probeDeterminism() {
  ...
  if (a.blocks.length !== b.blocks.length) {
    fail(`seoul: two builds differ in block count (${a.blocks.length} vs ${b.blocks.length})`);
    return;
  }
  const fa = fingerprint(a), fb = fingerprint(b);
  if (fa !== fb) fail(`seoul: two builds differ structurally (fingerprint ${fa} vs ${fb})`);
...
function probeWinnable(sim) {
  ...
  if (left > 0) fail(`seoul: ${left} block(s) survived a full clear — the harness is not clearing the city`);
  else if (!sim.won) {
    fail(`seoul: consumed all ${sim.blocks.length} blocks and won is still false (shortfall ${shortfall.toExponential(3)} vs epsilon ${epsilon.toExponential(3)})`);
  } else {
...
console.log('--- Validating Seoul Han River & N Seoul Tower ---');
const sim = newSim();
...
console.log(`\n  seoul sandbox: blocks=${sim.blocks.length} mass=${Math.round(sim.blocks.reduce((acc, b) => acc + (b.mass || 1), 0))} blockers=${sim.cameraBlockers.length}`);
