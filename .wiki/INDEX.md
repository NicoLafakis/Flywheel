...
> code and vocabulary — see [glossary.md](glossary.md)). Updated 2026-09-08.
> 
> **Implemented in this workspace:**
> - **24 playable cities out of 29 catalog entries**: verified from `js/citycatalog.js` on 2026-09-08. Rio de Janeiro, Buenos Aires, Mexico City, San Francisco and Toronto remain in development. HK2 is a separate experimental sandbox.
...
> - **Cloud Progress Sync (`js/cloud/`, `api/progress/`)**: Coins, skins, stars and upgrades follow the signed-in player across devices — a merged progress document, a server-side coin plausibility fence, and an offline-safe debounced sync queue (behind `FW_PROGRESS_SYNC`).
...
**Deployment boundary (2026-09-08):** `www.playflywheel.com` currently runs
`Flywheel-v2` commit `28805e1`. The capabilities above describe this legacy
repository; they are not a statement about that successor deployment. Current
remediation status and preview evidence are in
[the acceptance audit](plans/bug-remediation-2026-09-08.md#acceptance-audit).
...
| [modules/](modules/) | Per-module docs with `covers:` globs: [api](modules/api.md), [audio](modules/audio.md), [campaign](modules/campaign.md), [citygen](modules/citygen.md), [cloud](modules/cloud.md), [multiplayer](modules/multiplayer.md), [powerups](modules/powerups.md), [sim](modules/sim.md), [render](modules/render.md), [ui](modules/ui.md), [voxel](modules/voxel.md) |
| [plans/](plans/cloud-progress-sync.md) | [cloud-progress-sync](plans/cloud-progress-sync.md) — **built 2026-08-17** (tasks 1–17 complete; behind `FW_PROGRESS_SYNC` on Vercel). Coins, skins, stars and upgrades follow the signed-in player across devices: `player_progress` table, `/api/progress/pull\|push`, merge rules, coin fence. See [modules/cloud.md](modules/cloud.md) for the shipped shape and [ADR-0021](adr/0021-progress-is-a-merged-document-coins-are-fenced-not-trusted.md) for the decision record. [validator-optimization](plans/validator-optimization.md) — **plan, not built** (rewritten 2026-08-20). Cuts the 20–30 min full run by budgeting the Cambridge/Chicago excursions by work rather than sim-seconds (the actual long pole), then per-child scene preload, concurrent syntax check, `--fast`/`--city` aliases over `FW_VALIDATE_SECTIONS`, modularization last. Guardrails: empty scope refuses, refactored guards re-proved against the bad commit, no full-suite run as a gate for the work. [Developer Agent Request — North Star Sandbox Vertical Slice](plans/Developer%20Agent%20Request%20%E2%80%94%20Build%20the%20Flywheel%20North%20Star%20Sandbox%20Vertical%20Slice.md) — the creative brief behind the Sprocket storyline and Global Campaign (landed with the Sydney commit); brief, not a status doc |
| [adr/](adr/) | Architecture Decision Records |
| [runbooks/](runbooks/) | Run/validate/deploy playbooks |
| [findings/](findings/RCA-2026-08-11-skyscraper-launch-and-hanging-debris.md) | Root-cause analyses: [RCA-2026-08-11 skyscraper launch and hanging debris](findings/RCA-2026-08-11-skyscraper-launch-and-hanging-debris.md) (roof-snap teleport in the debris landing test, wall-scrape vy bounce, per-material gravity — **resolved, fixed by commit 235c82d the same day**); [RCA-2026-08-11 cambridge validator stall](findings/RCA-2026-08-11-cambridge-validator-stall.md) (the 780 s Cambridge excursion hits superlinear debris churn on the untiered physics — **resolved 2026-08-13**: root cause was unretirable jammed debris, fixed by T-402/[ADR-0018](adr/0018-debris-retires-on-proven-stationarity.md), and the validator is now a parallel orchestrator that completes end to end); [RCA-2026-08-17 eat SFX and voxel event audio](findings/RCA-2026-08-17-eat-sfx-and-voxel-event-audio.md) (commit `8c3c85d` deleted `js/main.js`'s one `audio.handleEvent(ev)` while adding the multiplayer `isLocalHole` guard, so the shipped voxel path never requests the eat gulps, the combo ladder, SIZE stings, milestones, the derailment or the tornado; assets and registry are correct, the caller is gone; **resolved 2026-08-17** — the guard is now a three-way (`!ev.hole || isLocalHole`, plus a quiet rival quake), because gating on hole ownership alone would have left every world-scoped event — the tornado, the derailment, collapse crashes, power-up spawns — silent in multiplayer; a real 5400-tick `VoxelSandboxSim` run now requests 813 gulps against 0 before, and `tools/sfx-event-guard.test.mjs` watches the seam by executing the guard text lifted from the shipped file rather than re-implementing it); [RCA-2026-08-17 chicago excursion red since the speed retune](findings/RCA-2026-08-17-chicago-excursion-red-since-speed-retune.md) (`e8b84ae` bundled `SPEED_MULT` 1.4→1.8 with audio work; the excursion harness advances waypoints on the CLOCK, so a faster hole arrives early and parks — consumption fell 1433→878 and the combo chain shattered 1433→443, dropping Chicago under its SIZE 7 gate. Manhattan-district, brooklyn and cambridge show the same degradation without yet failing; **fixed 2026-08-17** — all nine excursions and `tools/chicago-probe.mjs` now share one arrival-driven, looping driver (`tools/route-driver.mjs`) whose `until` is a per-lap ceiling, every excursion is held to ≤2% parked ticks and prints its laps/idle/distance, and a new `speedInvariance` section requires a x2.00 hole to cover ≥1.25x the ground a x1.00 hole does. Manhattan-district 5→7 and boston 5→10 rose because the holes now eat more; no floor was lowered. The spec's consumption-monotonicity clause was retired on measurement, not weakened — per-metre yield is gravity-time-gated in the sim and falls with speed under every driver, which is also surfaced as a possible gameplay finding in section 6b); [RCA-2026-08-17 backdrop-filter captures fixed descendants](findings/RCA-2026-08-17-backdrop-filter-captures-fixed-descendants.md) (a `backdrop-filter` ancestor becomes the containing block for its `position: fixed` descendants, so anything fixed inside a `.screen` is positioned against that screen's scrolling content box instead of the viewport and scrolls away with it; the shop tab bar travelled 6087 px off a 390x844 phone leaving 4 of 5 categories unreachable — **fixed**; `#mp-countdown-modal` has the identical shape inside the scrolling `.mp-lobby-view` and hides the pre-match countdown — **open**; audit of every `position: fixed` rule and the resulting convention in the doc); [RCA-2026-08-17 level-start camera transition](findings/RCA-2026-08-17-level-start-camera-transition.md) (commit `8818c2d` turned `Screens.showPokemonEncounterModal` into a stub that calls `onSkip()` synchronously, inverting the timing contract `main.js:playNextPokemonSpawn` was written against: it shows the presentation first and arms the camera second, so the cinematic's canceller now runs before the cinematic exists and every level start arms an uncancellable 1.5 s power-up spawn cinematic on the first frame, measured overriding the establishing shot with a 213.1 m single-frame position cut, a 1076 deg/s whip-pan against the module's own 400 deg/s ceiling, and a permanent pitch leak. The owner's power-up hypothesis is CONFIRMED; the same hijack repeats mid-play on every 30 s respawn; five distinct discontinuities plus three state defects, with a fix spec, the blockers to the requested overhead choreography, and six RED-proven assertions. **Fixed 2026-08-17** — steps 1-5 of the fix spec are implemented: the spawn intro is gated on `reason === 'initial'` and on `introActive()` (the "ORBITAL DROP DETECTED" toast is preserved, only the camera cinematic is suppressed), both cinematics arm BEFORE their presentation and return an identity-checked cancellation token, release is continuous in all five channels with a geometric distance blend and the rig's own 400 deg/s yaw cap, `realDt` is floored at 0, and the intro is now a four-phase `'off' | 'hold' | 'rise' | 'dive'` machine that eases overhead to a capped 1.40 rad before diving — the choreography the owner asked for. Live on two scenes: worst single-frame camera travel 213.1 m -> 5.1 m (manhattan) and 207.9 m -> 7.6 m (brooklyn), peak yaw 1220 deg/s -> 42.5 deg/s, pitch settles at its 0.5400 base instead of leaking to 0.5200, and 185 of 350 hijacked frames -> 0. Watched by `tools/cinematic-arming-guard.test.mjs` (73 assertions, 34 RED on the pre-fix tree); the quake cutscene's authored INTERNAL hard cuts are a named open item in [modules/render](modules/render.md)). [RCA-2026-08-20 cross-device zero progress](findings/RCA-2026-08-20-cross-device-zero-progress.md) (`registerPlayer`/`loginPlayer`/`claimName`/`renamePlayer` in `js/board/player.js` each whitelist a few expected 4xx outcomes and silently fabricate a `local-*` "claimed" identity under the player's typed name on ANY other failure, including a `404 PLAYER_NOT_FOUND` login.mjs returns for a name with no server-side account — so a second device can look "signed in" under the right name while holding zero real progress, with no visible cloud-dot difference from a genuine sync; the same mechanism was already caught in the 2026-08-17 smoke runbook's observation #2. **Fixed in `20ae91c`; regression suite rechecked 2026-09-08** — fix spec covers all four call sites plus a distinct "not really connected" UI state); [RCA-2026-08-20 The Lab: post-level buttons dead, then pause-menu RESTART hangs at 98%](findings/RCA-2026-08-20-the-lab-post-level-stuck-restart.md) (`js/main.js` calls `tutorialManager.unmount()` at two sites, `js/main.js:1104` inside the shared `teardownWorld()` and `js/main.js:1014` inside `startVoxelSandbox`, but `TutorialManager` (`js/ui/tutorial.js`) has only ever had a `teardown()` method — a naming mismatch as old as the class itself, latent until a fresh/tutorial-incomplete player exercises The Lab's teardown path, since `TutorialManager` is constructed only for `scene === 'gallery'`. The throw kills MAIN MENU and CHANGE MAP identically (both share the `onContinue` callback that calls `teardownWorld()` first) and, independently, poisons pause-menu RESTART: `buildSandbox()`'s own `teardownWorld()` throw is caught by `failSceneLaunch`, whose own recovery-path `teardownWorld()` call throws the same error uncaught, so `screens.showCitySelect()` never runs and the cosmetic, time-scripted loading bar is left frozen at its scripted ~98% ceiling forever. Confirmed live against production with two independent Playwright reproductions (dead results click; mid-game pause RESTART with no results screen involved) — one root cause, two exposure points, not a state-corruption chain. **Fixed in `20ae91c`; regression suite rechecked 2026-09-08** — fix spec covers both call sites plus a defense-in-depth per-subsystem try/catch for `teardownWorld()`, since this is the second time an unguarded first-statement throw in a shared callback has killed every button that shares it (see `d20c64e`, 2026-08-06, same symptom, different cause)). [RCA-2026-08-24 debris jiggle never settles](findings/RCA-2026-08-24-debris-jiggle-never-settles.md) (fallen debris held up by the contact separator is never `_grounded`: the 1.02 separation skin parks it a fraction of a millimetre above the zero-tolerance landing test, so every retirement path, walk sleep, T-402 jam latch and `_capDebris` alike, is ineligible forever, measured 243 permanently awake bodies on Boston; a secondary full-pen downward push on grounded bodies pumps the visible ~1 m jiggle; ablation-proven (position-proof latch 243→2, plus grounded-push clamp →0), **fixed 2026-08-25** riding the tornado rework's RANKED_SIM_VERSION 3→4: `_sepFloor` solver-support stamp + jam-latch alternative eligibility, grounded bodies never pushed below support; pinned by `tools/debris-settle.test.mjs`, post-storm sustained cost 5.700 → 0.002 ms/step); [RCA-2026-08-25 partner skins render identical](findings/RCA-2026-08-25-partner-skins-render-identical.md) (7 of 8 `docs/skins/partner-*.png` byte-identical to `classic.png` after the 2026-08-24 skins-rework regeneration — **not a defect**: the seven were withdrawn 2026-08-17 by commit `6824327` and `resolveAvailableRow` renders them as Classic by design; the committed tiles dated from 2026-08-07 and the rework's re-bake was the first since the gate landed, so the docs merely caught up. Live-app proof plus a clean skins-vs-district revert map; the skinsheet red-exit publication defect is fixed in `20ae91c`; its atomicity regression suite passed again 2026-09-08). Performance reviews: [PERF-2026-08-17 runtime performance review](findings/PERF-2026-08-17-runtime-performance-review.md) (seven proposed optimisations graded on code evidence — shader precompile, particle Mesh pooling and a ~9 MB service-worker precache are clean wins; the music preload is blocked on the single shared `Audio()` element; size-growth interpolation has no step to smooth; a sim Web Worker is declined on invariants, boundary cost and a frame of added latency); 2026-09-25 performance findings: [framerate](findings/PERF-2026-09-25-framerate.md) (collapse debris physics causes every attributed hitch; HIGH was uncapped), [load times](findings/PERF-2026-09-25-load-times.md) (per-fine-cell grid dominates city load; title waited on the Brooklyn backdrop; false boot "LOAD ERROR"), [geometry audit](findings/PERF-2026-09-25-geometry-audit.md). Step-1 fixes shipped 2026-09-25 (see STATUS) |
