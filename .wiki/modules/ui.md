...
| `js/ui/screens.js` | Title (branded landing over a live city backdrop: sprocket + `FLYWHEEL` wordmark + tagline plate, PLAY BROOKLYN CTA, an always-visible player status strip above city choices with Player Login / identity chip, Highest Score overall, and the graphic segmented coin progress meter toward the next skin level; a `FREE_PLAY`-driven voxel-scene chip shelf with per-city cleared/best records, one silhouetted locked card carrying its unlock price in words, THE RUN / RECORDS utilities, a demoted SHOP/SETTINGS row, and a `.fw-foot` footer pairing the CC0 sound manifest with the PRIVACY/TERMS legal line), shop, results, pause (two-step confirms for run-discarding buttons), mechanic intro |
