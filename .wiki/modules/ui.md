...
| `js/ui/menuscene.js` | The live city behind the landing screen — the same `VoxelSandboxSim` + `VoxelWorld3D` + `ChaseCamera` trio the sandbox mounts, on the same canvas, on autopilot (held establishing orbit, never released; a scripted heading sweep drives the hole so the skyline is actively being eaten). Scheduled, never blocking: `startMenuScene` only arms a timer (and since 2026-09-25 the build also waits until `#boot-splash` is gone; cold boot no longer builds it first, so the menu appears without waiting on Brooklyn and the backdrop fades in behind it), `tickMenuScene` is folded into `main.js`'s single rAF loop, and `stopMenuScene` disposes from `teardownWorld` before any game world claims the canvas |
...
| `index.html` | Canvas, HUD skeleton, importmap (three vendored same-origin, `js/vendor/three.module.js` — see `architecture.md`'s Boot section and [ADR-0014](../adr/0014-vendored-same-origin-runtime.md)), inline boot watchdog (progress-aware since 2026-09-25: it fails only after 25 s with no new milestone and no newly fetched file, so a slow boot is never reported as "LOAD ERROR"; `tools/boot-watchdog.test.mjs`) |
