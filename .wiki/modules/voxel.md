...
| `js/voxelscene-singapore.js` | `buildSingapore(sim)`: Marina Bay Sands and the Shoppes podium, the Supertree grove and biome domes, the Merlion and the Esplanade shells — ACT I chapter 3, `PLAYABLE` since 2026-08-19 (22,000 blocks / 36,474 mass / 571 camera blockers, matching `js/citycatalog.js` exactly). Notable: **the hole's spawn is hard-coded at (0, 16) for every scene** (`js/voxelsim.js`), so that square metre has to be solid promenade — the budget close-out course originally ran a lane straight through it and four 0.5 m cobbles were `consumed` on the opening frames, which reports through BOTH arms of `probeIdleStability` (its non-static arm counts `consumed`) and reads as a collapse that never happened. **Water paints over plaza**, so the Bayfront apron starts exactly at the bay's east edge (x 22) rather than 4 m inside it; the bay is 40 x 28 with its south shore at z 14, and the promenade boardwalk carries the spawn. The budget close-out course (§12) is the other half of that lesson and the sharper half: it laid 1,241 stones *northward from the shore into the bay* and stayed green, because `probeWaterOverSurfaces` compares DECOR rects and **a physical block standing in water is not a decor rect** — nothing in the suite looks at it. It rendered as a flat grey mat floating on Marina Bay. The lanes now walk SOUTH across the boardwalk deck (x -18..18, z 14..24, capacity 1,340 against 1,241 needed) with three skips: `sim.grid` occupancy, because writing over a promenade prop GHOSTS it rather than colliding; a keep-out read off `sim.hole`; and the water rects themselves. Validated by the `singapore` section in `tools/validate.mjs` (the shared probe pass) plus `tools/validate-singapore.mjs` (exact declared count, dead voxelkit imports, pure-sim boundary, determinism, winnable residual) — the same split Sydney uses. `tools/pw/singapore-shots.mjs` is the browser loop, and its repair is the lesson worth keeping: it and `tools/validate-singapore.mjs` were BOTH callers of the retired `VoxelSandboxSim.prototype._buildScene` seam, which only ever worked while the scene was *unregistered* — a registered-but-unloaded id throws by name instead of falling through. Registering the scene broke both, and only one got fixed; the Playwright half then died on a 60 s `waitForFunction` timeout and was found by RUNNING it, not by reading it. **After retiring half of a two-sided seam the question is never "does the caller work" but "how many callers were there."** Both now `await loadScene('singapore')` and carry no patch. The Playwright half additionally reads its frame with `canvas.toDataURL()` inside the render's own `evaluate`, because `page.screenshot()` photographs the compositor and cannot capture a manually flown camera. The gallery fall-through itself is still a legitimate seam for an id the registry does not know — `tools/probe-aniso.mjs` uses it and runs |
