...
- [ ] **T1 — Per-slot color identity, exported once.** *Trivial.* One module
  ...
  mapping. → CC-3.
- [ ] **T2 — Attribution record from live events.** *Trivial.* Maintain
  ...
  AC-01.8, CC-1, CC-2.
- [ ] **T3 — Eater identity survives the keyframe.** *Small.* The one wire
...
- [ ] **T4 — Crater tinting.** *Small.* Render the exposed footprint of every
...
- [ ] **T5 — Tug-of-war bar.** *Trivial.* One N-segment bar over `rawMass`
...
- [ ] **T7 — Off-screen rival chevron.** *Small.* Edge-projected chevron per
...
- [ ] **T8 — Beat detection.** *Trivial.* Pure detectors over sim/snapshot
...
- [ ] **T9 — Feed copy table + queue source.** *Trivial.* Callout copy as a
...
- [ ] **T10 — Match-end territory reveal.** *Medium.* Camera pull-up over the
