...
- [ ] **T-202 — Staging Lobby Manager & Start Control**: Implement `js/multiplayer/lobby.js` managing room creation, joining, invite link encoding (`?room=CODE`), capacity bounds, the two countdown entry points (host start at any count $\ge 2$; unanimous non-host vote after $\ge 45$s of host idle with $\ge 3$ seated), countdown cancellation on peer drop, start-vote reset on any roster change or host activity, and ephemeral in-memory chat log management. Test suites: `tools/multiplayer-lobby.test.mjs`, `tools/lobby-start-control.test.mjs`. *(Originally specified as an automatic 3.0s countdown on full capacity; the auto-start was removed 2026-08-17 by owner decision — see REQ-MP-06.)*
